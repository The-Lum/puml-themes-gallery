...
# for all PlantUML theme (from `script/theme.yml`)
...
my @files = glob("collections/_stdlibs/input/*.puml");
...
foreach my $t (@themes) {
    foreach my $f (@files) {
        ...
        $file_no_ext =~ s|^.*/||;
        # Output File
        open(my $fho, '>:encoding(UTF-8)', 'gallery/themed-input/' . 'stdlib-' . $file_no_ext . '-' . $t . '.puml');
        print $fho $h;
        while (<$fh>) {
            $_ .= "!theme $t\n" if $. == 1;
            print $fho $_;
